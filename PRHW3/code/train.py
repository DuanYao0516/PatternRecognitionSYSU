...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            inputs = interleave(torch.cat((inputs_l, inputs_u_1, inputs_u_2)), 2*args.mu+1).to(args.device)
            targets_l = targets_l.to(args.device)
            logits = model(inputs)
            logits = de_interleave(logits, 2*args.mu+1)
            logits_l = logits[:batch_size]
            logits_u_1, logits_u_2 = logits[batch_size:].chunk(2)
            del logits

            Ll = F.cross_entropy(logits_l, targets_l, reduction='mean')

            pseudo_label = torch.softmax(logits_u_1.detach() / args.T, dim=-1)
            max_probs, targets_u = torch.max(pseudo_label, dim=-1)
            mask = max_probs.ge(args.threshold).float()

            Lu = (F.cross_entropy(logits_u_2, targets_u, reduction='none') * mask).mean()
            ...
            p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Loss: {loss:.4f}. Loss_x: {loss_x:.4f}. Loss_u: {loss_u:.4f}. Mask: {mask:.2f}. ".format(
                ...
                batch=batch_idx + 1, iter=args.eval_step,
                lr=scheduler.get_last_lr()[0], loss=losses.avg, loss_x=losses_l.avg,
                loss_u=losses_u.avg, mask=mask_probs.avg))
            p_bar.update()
        ...
        if args.local_rank in [-1, 0]:
            ...
            args.writer.add_scalar('train/2.train_loss_x', losses_l.avg, epoch)
            ...
            save_checkpoint({
                ...
                'acc': test_acc, 'best_acc': best_acc,
        ...
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            if not args.no_progress:
                test_loader.set_description(
                    "Test Iter: {batch:4}/{iter:4}. Loss: {loss:.4f}. top1: {top1:.2f}. top5: {top5:.2f}. ".format(
                        batch=batch_idx + 1,
